...
def get_mask(value: list):
    """
    Функция кодировки номера карты и счета
    :param value: передается список с наименованием и номером карты, либо именем "Счет" и номером счета
                    Пример списков:
    ...
    :return: если длина строки равна 16 и список состоит из 2-х элементов,
             ...
             если  если длина строки равна 16 и список состоит из 3-х элементов,
                то возвращается строка содержащая зашифрованный номер карты. Пример: "Visa Gold 1234 12** **** 1234"
             иначе возвращается строка содержащая зашифрованный номер счета. Пример: "Счет **1232"
    """

    if len(value[-1]) == 16 and len(value) == 2:
        new_card = "".join(value[0] + " " + (value[-1][:4] + " " + value[-1][4:6]
                                             + "** " + "**** " + value[-1][-4:]))
        return new_card
    elif len(value[-1]) == 16 and len(value) == 3:
